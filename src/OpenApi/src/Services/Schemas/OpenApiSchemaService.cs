    ...
    IOptionsMonitor<OpenApiOptions> optionsMonitor)
{
    ...
    private readonly JsonSchemaExporterOptions _configuration = new()
    {
        ...
        TransformSchemaNode = (context, schema) =>
        {
            ...
            if (context.PropertyInfo?.AttributeProvider?.GetCustomAttributes(inherit: false).OfType<ValidationAttribute>() is { } validationAttributes)
            {
                schema.ApplyValidationAttributes(validationAttributes);
            }
            if (context.PropertyInfo?.AttributeProvider?.GetCustomAttributes(inherit: false).OfType<DefaultValueAttribute>().LastOrDefault() is DefaultValueAttribute defaultValueAttribute)
            {
                schema.ApplyDefaultValue(defaultValueAttribute.Value, context.TypeInfo);
            }
            if (context.PropertyInfo?.AttributeProvider?.GetCustomAttributes(inherit: false).OfType<DescriptionAttribute>().LastOrDefault() is DescriptionAttribute descriptionAttribute)
            {
